    ...
    public class RDTransactionDAL : SettingsDAL, IRDTransaction
    {
        ...
        public async Task<List<RdMembersandContactDetails>> GetRDMembers(string Contacttype, string MemberType, string ConnectionString)
        {
            ...
            await Task.Run(() =>
            {
                try
                {
                    using (NpgsqlDataReader dr = NPGSqlHelper.ExecuteReader(ConnectionString, CommandType.Text, "select memberid,membercode,membername,contactid,contactreferenceid from tblmstmembers where  contacttype ='" + Contacttype + "' and membertype = '" + MemberType + "' and statusid = " + Convert.ToInt32(Status.Active) + " order by membername; "))
                    {
                        while (dr.Read())
                        {
